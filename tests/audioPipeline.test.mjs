...
test('rejects invalid timestamps and unintended overlap', async () => {
  ...
  const overlapping = [
    { key: 'audio_0', start: '00:00:01.000', end: '00:00:03.500', segmentId: 'seg-1' },
    { key: 'audio_1', start: '00:00:03.500', end: '00:00:05.000', segmentId: 'seg-2' }
  ];
  const twoSegments = await createSegments([{ ttsDuration: 3.4 }, { ttsDuration: 1 }]);
...
test('preserves intentional source overlap while rejecting TTS overrun', async () => {
  const intentional = [
    ...
    { key: 'audio_1', start: '00:00:02.000', end: '00:00:05.000' }
  ];
  ...
  assert.equal(segments[0].ttsDuration > 2.1, true);
  assert.equal(segments[1].placementSeconds, 2);
